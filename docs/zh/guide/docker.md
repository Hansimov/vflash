...
需要 Linux AMD64、Docker Compose v2、NVIDIA Container Toolkit，以及兼容镜像内 CUDA 13.0 运行时的 NVIDIA 驱动。另外还需要一张[受支持的显卡](./profiles)和四类[运行资源](../reference/runtime-assets)。
...
上述命令会从当前源码在本地构建 `vflash:0.1.0a1`。首次构建需要下载固定版本的运行依赖。模型资源以只读方式挂载，输出和内核缓存使用独立的可写存储。
