...
::: warning 镜像版本
最新预构建 Docker 镜像仍为 **0.3.2**。0.5.2 发布源码和 wheel；如需 Base16 关键帧、
五至十秒请求和新的精确双卡重排，请在本地构建 `runtime` 或 `pipeline` target。
不得从 0.3.2 镜像推导 0.5.2 能力。
:::
...
带版本号的镜像已发布到 [Docker Hub](https://hub.docker.com/r/hansimov/vflash/tags)，不可变摘要见[镜像清单](https://github.com/Hansimov/vflash/blob/v0.3.2/docker/images.json)。模型只读挂载，输出和内核缓存使用独立的可写存储。需要本地构建 0.5.2 时，执行 `docker build --target runtime -t vflash:0.5.2 .`，然后在环境文件中选择该镜像。
...
如需从 v0.5.2 标签源码构建完整镜像，可执行 `docker build --target pipeline -t vflash:0.5.2-pipeline .`，并将命令中的镜像名替换为本地名称。
