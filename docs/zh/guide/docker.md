...
::: warning 镜像版本
最新预构建 Docker 镜像仍为 **0.3.2**。0.5.1 发布源码和 wheel；如需 Base16 关键帧、
五至十秒请求和新的精确双卡重排，请在本地构建 `runtime` 或 `pipeline` target。
不得从 0.3.2 镜像推导 0.5.1 能力。
:::
...
带版本号的镜像已发布到 [Docker Hub](https://hub.docker.com/r/hansimov/vflash/tags)，不可变摘要见[镜像清单](https://github.com/Hansimov/vflash/blob/v0.3.2/docker/images.json)。模型只读挂载，输出和内核缓存使用独立的可写存储。需要本地构建 0.5.1 时，执行 `docker build --target runtime -t vflash:0.5.1 .`，然后在环境文件中选择该镜像。
...
```bash
docker build --target pipeline -t vflash:0.5.1-pipeline .
...
docker run --rm --runtime=runc -e NVIDIA_VISIBLE_DEVICES=void \
  ...
  vflash:0.5.1-pipeline prepare-pipeline \
...
docker run --rm --gpus device=0 --shm-size 4g \
  ...
  -v "$PWD/inputs:/inputs:ro" -v "$PWD/outputs:/outputs:rw" \
  -v "$PWD/cache:/cache:rw" \
  vflash:0.5.1-pipeline generate \
...
如需从 v0.5.1 标签源码构建完整镜像，可执行 `docker build --target pipeline -t vflash:0.5.1-pipeline .`，并将命令中的镜像名替换为本地名称。
...
已发布镜像复用不可变的 0.3.1 镜像层，安装 0.3.2 wheel。[发行附件](https://github.com/Hansimov/vflash/releases/tag/v0.3.2)包含该 wheel 和 `Dockerfile.release`；[清单](https://github.com/Hansimov/vflash/blob/v0.3.2/docker/images.json)记录其摘要、依赖镜像与验证范围。这样不必重新下载未变更的依赖；上面的源码 Dockerfile 仍提供完整构建方法。 复现已发布的小增量镜像时，先检出清单中的 `implementation_revision`，把发行 wheel 放入 `dist/`，再使用 `Dockerfile.release`。最终标签补充文档和镜像清单，运行源码不变。
