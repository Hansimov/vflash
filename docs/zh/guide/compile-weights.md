...
编译器从固定 H3 官方权重和匹配的 LoRA 准备运行资产，支持 SM86/SM89 的 Ref4 v0.1，以及 SM89 的 T2VA Base4 v1.0。不需要捕获请求、参考图片或其他项目的实验资产。原有 SM89 配置的全部 50 层、调度和辅助张量均已与固定控制逐位核对。SM86 在完整生成前，还单独对照了同架构官方时间编码和全部 50 层调制计算。
