...
此开发分支增加了从提示词和一张参考图生成 MP4 的 Python 接口。首个目标是单张 48 GiB SM89 显卡上的 Ref2VA Turbo4，已在 928 × 512 下完成连续两次生成及取消请求的完整 GPU 集成验证。它尚未包含在当前仅输出 latent 的公开发行版中，仍需提供准备好的原生资产；独立编译流程正在补齐。
