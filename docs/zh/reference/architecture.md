...
Vflash 将已经编码的 H3 请求转换为视频和音频潜变量。Transformer、LoRA 残差、调度更新和输出头由自己的 PyTorch、Triton 运行时执行，不导入 LightX2V 推理框架。
...
条件包包含编码后的文本与参考素材、序列布局和初始噪声。提示词与参考图编码发生在这条边界之前，VAE 解码和 MP4 封装发生在之后；这些前后处理暂未包含在公开包中。
