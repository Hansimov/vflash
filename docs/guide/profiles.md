...
`block-ring` now groups weight tensors into larger pinned host allocations, reducing unused space without changing the weight data or its logical byte count.
...
The **3080 profile** streams blocks from pinned system memory while the GPU computes. For a 928 × 512, 124-frame, four-step workload, we recommend **at least 64 GiB of available host RAM per worker**, with additional room for the operating system and other applications. Larger inputs and concurrent workers need separate capacity measurements; 64 GiB is not a universal capacity guarantee. Faster host-to-device transfers and sufficient RAM matter as well as GPU compute.
