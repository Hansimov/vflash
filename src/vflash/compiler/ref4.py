"""Compile the fixed BF16 Ref4 runtime pack directly from official weights.
...
from vflash.model_assets import canonical_sha256, ref4_weights_source
...
from .assets import PreparedRef4Weights, load_prepared_ref4_weights
from .math import adaln_table, time_embeddings

SPEC = H3Spec(50, 5376, 56, 128, 14336, 2688, 24, 32, (1, 2, 2))
TARGET = resolve_h3_artifact_target("rtx4090-48g-sm89-bf16-resident")
...
def validate_ref4_weight_headers(prepared: PreparedRef4Weights) -> dict[str, int]:
    """Check every required tensor's name, dtype and shape without touching CUDA."""
    checked = load_prepared_ref4_weights(prepared.receipt)
    if checked != prepared:
...
def _compile_block(
    ...
    table = adaln_table(
        timing["time_embeddings"], (2, 3, 3, 3), weight, bias, modalities=3, modulations=6
    ).cpu()
    del weight, bias
...
def _write_assets(
    ...
    artifact_id = "h3-runtime-ref4-bf16-sm89-" + canonical_sha256(source)[:12]
    ...
    timing = time_embeddings(checkpoint.load, device)
    records = []
    for index in range(SPEC.num_layers):
        block = _compile_block(index, checkpoint, adapter, timing, device)
    ...
    overlay = {
        "schema_version": 2,
        "overlay_id": "h3-schedule-ref4-" + canonical_sha256(source)[:12],
        "created_at": created_at,
        ...
        "schedule": H3NativeSchedule.shifted_linear(4).to_mapping(),
        "source": {
...
def compile_ref4_assets(
    ...
    *,
    ...
    """Compile in a fresh process on an exclusively assigned SM89 GPU.
    ...
    validate_ref4_weight_headers(prepared)
    if not isinstance(device, NvidiaDevice) or device.compute_capability != "8.9":
        raise ContractError("the Ref4 weights compiler requires an explicit SM89 device")
    ...
    if shutil.disk_usage(destination.parent).free < 48 * 1024**3:
        raise ContractError("compiling Ref4 requires at least 48 GiB of free output space")
    if getattr(ctypes.CDLL(None), "renameat2", None) is None:
        raise ContractError("the weights compiler requires Linux renameat2")
    os.environ["CUDA_VISIBLE_DEVICES"] = device.uuid
    if torch.cuda.get_device_capability(0) != (8, 9):
        raise ContractError("the visible CUDA device differs from the SM89 compile target")
